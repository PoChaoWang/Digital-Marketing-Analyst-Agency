...
- 對固定 metric lookup、aggregation、period comparison、platform summary、GA4 landing page quality 這類可重複計算任務，若 `recipes/` 中已有對應 recipe，必須優先使用 `scripts/run-recipe.mjs`，不得手動彙總 raw rows。
...
5. 若有 matching recipe，先使用 `scripts/run-recipe.mjs` 產出 compact JSON。
