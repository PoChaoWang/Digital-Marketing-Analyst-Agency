...
- Production 資料清洗、360 table 建置與 period compare 使用 Python + SQL。
- `scripts/run-recipe.mjs` 保留為 legacy / development-only，用於 Mock MCP 與舊 recipe 驗證。
- 新的 production analysis 腳本預設延續 Python + SQL，除非有明確理由改用其他語言。

執行 SQL-backed combiner：
...
指標規則：CVR、CPA、ROAS、ROI、conversion、revenue 相關計算一律使用 360 table 裡的 GA4 欄位。平台回報的 conversions / revenue 保留作為稽核與差異檢查。
...
- `docs/360_TABLE_WORKFLOW.md`：SQL-backed 360 table 與 period compare 流程。
